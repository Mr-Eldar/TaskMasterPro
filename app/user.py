...
async def cmd_start(message: Message):
    await message.answer('🚀 Добро пожаловать в <b>TaskMaster Pro</b> — твой умный помощник для управления задачами!\n\n'
                        ...
                        '🤖 <b>Скоро здесь появится ещё больше крутых функций...</b>',
                         reply_markup=kb.start_kb,
                         parse_mode=ParseMode.HTML)
    await message.reply('<b>👇 Нажмите на синию кнопку меню чтобы начать или воспользуйтесь кнопками.</b>', parse_mode=ParseMode.HTML)
...
async def cmd_tasks(event: Message | CallbackQuery):
    if isinstance(event, Message):
        ...
        await event.answer('📋 Ваши текущие планы\n\n'
                        'Здесь отображается полный список активных планов. Для управления используйте соответствующие команды.\n\n'
                        'ℹ️ <i>Чтобы добавить, изменить или удалить план — воспользуйтесь командой <b>Упарвление планами</b> или <b>/task_management:</b></i>.',
        ...
        if user_id is None:
            ...
            return
        await event.answer()
        await event.message.edit_text('📋 Ваши текущие планы\n\n'
                        'Здесь отображается полный список активных планов. Для управления используйте соответствующие команды.\n\n'
                        'ℹ️ <i>Чтобы добавить, изменить или удалить план — воспользуйтесь командой <b>Упарвление планами</b> или <b>/task_management:</b></i>.',
                        reply_markup=await kb.tasks(user_id), parse_mode=ParseMode.HTML)
...
@user.message(F.text == 'Управление планами ⚙️')
async def cmd_tasks_management(message: Message):
    ...
    await message.answer('🎯 <b>Управление задачами и планами</b>\n\n'
                        'Здесь вы можете полностью контролировать свои планы и ежедневные задачи.\n\n'
                        '🔹 <b>Добавление</b>\n'
                        'План — долгосрочная цель (например, "Изучить TypeScript")\n'
                        'Задача — конкретное действие на день (например, "Практика типов с 10:00 до 12:00")\n\n'
                        '🔹 <b>Изменение</b>\n'
...
async def cmd_get_work_tasks(callback: CallbackQuery):
    ...
    await callback.message.answer('📅 <b>Задачи на сегодня</b>\n\n'
                                        'Здесь отображаются все ваши запланированные задачи.\n'
                                        'Чтобы добавить или изменить задачу, используйте команду <b>/task_management</b>.',
